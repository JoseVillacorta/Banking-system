...
public class AccountHandler {
    ...
    public Mono<ServerResponse> findById(ServerRequest request){
        ...
        return accountService.findById(id)
                ...
                .switchIfEmpty(ServerResponse.status(HttpStatus.NOT_FOUND)
                        .bodyValue(ApiResponse.error("Account not found", HttpStatus.NOT_FOUND.value())));
    }
    ...
    public Mono<ServerResponse> create(ServerRequest request){
        return request.bodyToMono(Account.class)
                ...
                .flatMap(a -> ServerResponse
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .bodyValue(ApiResponse.success(a, "Account created successfully", HttpStatus.CREATED.value())))
    ...
    public Mono<ServerResponse> transfer(ServerRequest request){
        return request.bodyToMono(TransferRequest.class)
                .flatMap(req -> accountService.transfer(req.getFromAccountId(), req.getToAccountId(), req.getAmount()))
                .then(ServerResponse.ok()
                        .contentType(MediaType.APPLICATION_JSON)
                        .bodyValue(ApiResponse.success(null, "Transferencia realizada exitosamente (ACID)", HttpStatus.OK.value())))
                // Si el stored Procedure escupe un RAISE EXCEPTION lo atajamos aqui
                .onErrorResume(e -> ServerResponse.status(HttpStatus.BAD_REQUEST)
                        .bodyValue(ApiResponse.error("Error en transferencia: " + e.getMessage(), HttpStatus.BAD_REQUEST.value())));
    }
